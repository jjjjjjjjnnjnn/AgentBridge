...
| 섹션 | 설명 |
|---------|-------------|
| [🎯 개요](#-개요) | AgentBridge가 무엇이고 왜 필요한지 |
| [✨ 기능](#-기능) | 현재 기능 |
...
**AgentBridge**는 AI 에이전트를 위한 오픈소스 조정 레이어입니다——Docker가 컨테이너를 위한 것처럼, AI 도구를 위한 것입니다.
...
AgentBridge는 각 AI CLI를 "터미널"——독립적으로 실행되는 워커로 취급합니다:
...
설정 파일 위치: `~/.agentbridge/config.yaml` (또는 `$AGENTBRIDGE_CONFIG_DIR/config.yaml`)
...
AgentBridge는 거인의 어깨 위에 서 있습니다. 다음 프로젝트에 깊은 감사를 드립니다:

### 🖥️ 터미널 플랫폼

| 플랫폼 | 감사의 말 |
|----------|--------|
| **[Claude Code](https://claude.ai)** — Anthropic 제공 | 주요 개발 플랫폼. AgentBridge는 Claude Code의 에이전트 오케스트레이션 기능을 사용하여 설계 및 구축되었습니다. [약관](https://www.anthropic.com/legal) · [개인정보](https://www.anthropic.com/privacy) |
| **[OpenCode](https://opencode.ai)** | 터미널 어댑터 대상 및 테스트 파트너. OpenCode CLI는 AgentBridge의 터미널 풀이 사용하는 실행 인터페이스를 제공합니다. |
...
AgentBridge README는 다음 언어로 제공됩니다:
...
<p align="center">
  <strong>AgentBridge</strong> — AI 에이전트를 위한 조정 레이어.<br>
